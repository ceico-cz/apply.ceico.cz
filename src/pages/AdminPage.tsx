...
export function AdminPage({ currentUser }: { currentUser: User }) {
  ...
  const [editingUser, setEditingUser] = useState<Pick<User, 'id' | 'email' | 'first_name' | 'last_name'> | null>(null)
  ...
  const updateName = useMutation({
    mutationFn: () => api<User>(`/admin/users/${editingUser!.id}/profile`, { method: 'PUT', body: JSON.stringify({ first_name: editingUser!.first_name, last_name: editingUser!.last_name }) }),
    onSuccess: async (updated) => {
      if (updated.id === currentUser.id) client.setQueryData<User>(['me'], (current) => current ? { ...current, first_name: updated.first_name, last_name: updated.last_name } : current)
      setEditingUser(null)
