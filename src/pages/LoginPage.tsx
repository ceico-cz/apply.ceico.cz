...
export function LoginPage() {
  ...
  return <section className="auth-page"><div className="auth-card"><div className="brand-mark large">C</div><div className="eyebrow">Secure access</div><h1>Sign in</h1>{providers.data?.pocket_id.enabled && <><p>Use your Phoebe identity for immediate access.</p><a className="button pocket-id-button full" href={pocketIdUrl}><span className="pocket-id-mark" aria-hidden="true">P</span>Continue with {providers.data.pocket_id.name}</a><div className="auth-divider"><span>or sign in without a password</span></div></>}<p>Enter your email and we’ll send a single-use sign-in link.</p>{result ? <div className="notice success"><strong>Check your inbox</strong><p>{result.message}</p>{result.development_url && <a href={result.development_url}>Development sign-in link →</a>}</div> : <form onSubmit={submit}><label>Email address<input required type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@institution.edu" /></label><button className="button primary full" disabled={busy}>{busy ? 'Sending…' : 'Email me a sign-in link'}</button></form>}<small>Email links expire after 20 minutes and can be used only once.</small></div></section>
}
