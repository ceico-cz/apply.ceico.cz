...
export function RefereePage() {
  ...
  const validRequestId = Boolean(requestId && /^\d+$/.test(requestId) && Number(requestId) > 0)
  const listQuery = useQuery<Request[]>({ queryKey: ['referee-requests'], queryFn: () => api('/referee/requests'), enabled: !isDetail })
  const detailQuery = useQuery<Request>({ queryKey: ['referee-request', requestId], queryFn: () => api(`/referee/requests/${requestId}`), enabled: isDetail && validRequestId, retry: false })
  const upload = async (id: number, file?: File) => { if (!file) return; const form = new FormData(); form.append('file', file); await api(`/referee/requests/${id}/letter`, { method: 'POST', body: form }); await Promise.all([client.invalidateQueries({ queryKey: ['referee-requests'] }), client.invalidateQueries({ queryKey: ['referee-request', String(id)] })]) }
  const requests = isDetail ? (detailQuery.data ? [detailQuery.data] : []) : listQuery.data
  const isLoading = isDetail ? detailQuery.isLoading : listQuery.isLoading
  return <section className="container page narrow"><div className="page-heading"><div><div className="eyebrow">Confidential references</div><h1>{isDetail ? 'Reference letter request' : 'Reference letter requests'}</h1></div>{isDetail && <Link className="button ghost" to="/dashboard">Back to dashboard</Link>}</div>{requests?.length ? requests.map((item) => <article className="panel reference-task" key={item.id}><div className="task-title"><div><small>Letter in support of</small><h2>{item.applicant}</h2><p>{item.campaign}</p></div><StatusPill status={item.status} /></div><div className="deadline-note inline"><small>Submit by</small><strong>{new Date(item.letter_deadline).toLocaleString()}</strong></div><p className="privacy-note">Your letter is confidential. The applicant can see that it was received but cannot view its contents.</p>{item.document && <div className="uploaded-reference"><span aria-hidden="true">✓</span><div><small>Uploaded reference letter</small><a href={`${API_URL}/api/v1/referee/requests/${item.id}/letter`} target="_blank" rel="noreferrer">{item.document.original_name}</a><small>{fileSize(item.document.size)} · uploaded {new Date(item.document.created_at).toLocaleString()}</small></div></div>}<label className={`button ${item.document ? 'secondary' : 'primary'} file-button`}>{item.document ? 'Replace letter' : 'Upload reference letter'}<input type="file" accept="application/pdf,.pdf" onChange={(e) => upload(item.id, e.target.files?.[0])} /></label></article>) : !isLoading && <EmptyState title={isDetail ? 'Reference request not found' : 'No reference requests'}>{isDetail ? 'This request does not belong to your verified email address, or it is no longer available.' : 'Requests sent to your verified email will appear here.'}</EmptyState>}</section>
}
