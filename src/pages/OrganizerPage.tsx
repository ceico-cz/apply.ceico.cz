...
const readyApplicationStatuses: ApplicationStatus[] = [ApplicationStatus.reviewReady, ApplicationStatus.overrideReady]
...
function CampaignWorkspace({ campaign }: { campaign: Campaign }) {
  ...
  const stats = useMemo(() => ({ total: query.data?.length || 0, ready: query.data?.filter((a) => readyApplicationStatuses.includes(a.status)).length || 0, awaiting: query.data?.filter((a) => a.status === ApplicationStatus.awaitingReferences).length || 0 }), [query.data])
...
function ApplicantDrawer({ application, campaign, canRecordDecisions, close, refresh }: { application: Application; campaign: Campaign; canRecordDecisions: boolean; close: () => void; refresh: () => void }) {
  ...
  const decision = async (outcome: string) => {
    if (!Object.values(DecisionOutcome).includes(outcome as DecisionOutcome)) return
    const decisionOutcome = outcome as DecisionOutcome
    await api(`/applications/${application.id}/decision`, { method: 'PUT', body: JSON.stringify({ outcome: decisionOutcome, notes: '' }) })
    if (window.confirm(`Decision recorded as ${decisionOutcome}. Send the outcome email now?`)) {
      await api(`/applications/${application.id}/decision/send`, { method: 'POST' })
      window.alert('Outcome email sent')
    }
  }
  const questionLabels = new Map(campaign.questions.map((question) => [question.id, question.label]))
  const displayValue = (value: string | string[] | boolean) => Array.isArray(value) ? value.join(', ') : typeof value === 'boolean' ? value ? 'Yes' : 'No' : value
  const profileEntries = Object.entries(application.profile).filter(([, value]) => value)
  return <div className="drawer-backdrop" onMouseDown={close}><aside className="drawer applicant-drawer" onMouseDown={(event) => event.stopPropagation()}><button type="button" className="drawer-close" aria-label="Close applicant details" onClick={close}>×</button><div className="eyebrow">Applicant #{application.id}</div><h2>{application.applicant.first_name} {application.applicant.last_name}</h2><p><a href={`mailto:${application.applicant.email}`}>{application.applicant.email}</a></p><div className="applicant-drawer-status"><StatusPill status={application.status} />{application.consent_at && <span>Consent recorded {new Date(application.consent_at).toLocaleString()}</span>}</div><section className="applicant-detail-section"><div className="applicant-detail-heading"><h3>Application details</h3>{application.documents.length > 0 && <a className="button secondary" href={`${API_URL}/api/v1/applications/${application.id}/packet`} target="_blank" rel="noreferrer">Open review packet</a>}</div><dl className="application-detail-grid">{profileEntries.map(([key, value]) => <div key={key}><dt>{fieldLabel(key)}</dt><dd>{value}</dd></div>)}<div><dt>Primary research</dt><dd>{application.primary_field_label || '—'}</dd></div><div><dt>Secondary research</dt><dd>{application.secondary_fields.map((field) => field.label).join(', ') || '—'}</dd></div>{application.other_research && <div><dt>Other research interests</dt><dd>{application.other_research}</dd></div>}</dl></section>{application.career.length > 0 && <section className="applicant-detail-section"><h3>Career history</h3><div className="career-history">{application.career.map((entry, index) => <dl key={index}>{Object.entries(entry).filter(([, value]) => value).map(([key, value]) => <div key={key}><dt>{fieldLabel(key)}</dt><dd>{value}</dd></div>)}</dl>)}</div></section>}{Object.keys(application.responses).length > 0 && <section className="applicant-detail-section"><h3>Application answers</h3><dl className="application-answers">{Object.entries(application.responses).map(([key, value]) => <div key={key}><dt>{questionLabels.get(key) || fieldLabel(key)}</dt><dd>{displayValue(value)}</dd></div>)}</dl></section>}<section className="applicant-detail-section"><h3>Documents</h3>{application.documents.length ? <div className="applicant-documents">{application.documents.map((document) => <a href={`${API_URL}/api/v1/applications/${application.id}/documents/${document.id}`} target="_blank" rel="noreferrer" key={document.id}><span aria-hidden="true">PDF</span><span><strong>{document.original_name}</strong><small>{fieldLabel(document.kind)} · {fileSize(document.size)}</small></span></a>)}</div> : <p className="muted">No documents uploaded.</p>}</section><section className="applicant-detail-section"><h3>Referees</h3>{application.referees.length ? <div className="applicant-referees">{application.referees.map((referee) => <div key={referee.id}><span><strong>{[referee.first_name, referee.last_name].filter(Boolean).join(' ') || referee.email}</strong><small>{referee.email}{referee.phone ? ` · ${referee.phone}` : ''}</small></span><StatusPill status={referee.status} /></div>)}</div> : <p className="muted">No referees added.</p>}</section>{!readyApplicationStatuses.includes(application.status) && <button className="button secondary full" onClick={override}>Admit incomplete application</button>}<form onSubmit={assign} className="drawer-section"><h3>Assign evaluator</h3><label className="field">Verified email<input required type="email" list={`campaign-evaluators-${campaign.id}`} value={email} onChange={(event) => setEmail(event.target.value)} placeholder="reviewer@institution.edu" autoComplete="off" /></label><button className="button primary full" disabled={!readyApplicationStatuses.includes(application.status)}>Assign</button></form>{canRecordDecisions && <div className="drawer-section"><h3>Final decision</h3><div className="decision-grid">{[DecisionOutcome.selected, DecisionOutcome.waitlisted, DecisionOutcome.rejected].map((item) => <button type="button" className="button ghost" onClick={() => decision(item)} key={item}>{item}</button>)}</div></div>}</aside></div>
}
